...
export const Navbar = () => {
  ...
  return (
    <div className="sticky top-0 left-0 right-0 z-50 bg-secondaryBackground max-w-[100%] mx-auto ">
      <Top /> {/* imported from the top component of the header */}
      <div className="flex flex-wrap justify-between items-center max-w-[90%] mx-auto px-4 text-lg rounded-lg md:bg-white py-2 my-2 bg-transparent ">
        <ul className="hidden md:flex space-x-8 font-semibold ">
          {navlink.map((links, i) => (       //mapping through the navigation links
            <li key={i}>     
              <Link to={links.url} data-aos=" ">
                {links.text}
              </Link>
            </li>
          ))}
        ...
        <div
          className={`md:hidden fixed left-0 top-0 w-[60%] h-full md:w-1/2 border-r text-greyDark border-gray-900 bg-headingColor ease-in-out duration-500 ${
            nav ? '' : 'left-[-100%]'
          }`}
        >
          <h1 className=" my-2 p-4 w-full text-3xl font-bold text-greyDark">  
